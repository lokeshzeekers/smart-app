...
export default function TrainerReviews() {
  ...
  useEffect(() => {
    setLoading(true);
    api.get('/trainer/review-queue').then(({ data }) => {
      setQueue(data.queue);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto pb-24">
      <header className="pt-6 pb-4 px-5">
        <h1 className="font-display font-semibold text-lg text-ink-900">SMArT Score for Review</h1>
        <p className="text-sm text-ink-500 mt-1">Pending submissions from your registered trainees</p>
      </header>
      ...
      <div className="px-5 space-y-3">
        ...
        {!loading && queue.length === 0 && (
          <p className="text-ink-300 text-sm text-center py-8">No submissions pending review.</p>
        )}
